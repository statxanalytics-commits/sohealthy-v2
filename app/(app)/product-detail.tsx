...
import { ClipboardList, Clock, Snowflake, Lightbulb, Bell, Leaf, Star } from 'lucide-react-native'
...
export default function ProductDetailScreen() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const { data: sel } = await supabase.from('product_selections').select('product_slug').eq('user_id', user.id).eq('is_active', true).order('selected_at', { ascending: false }).limit(1).single()
      if (sel?.product_slug) setSlug(sel.product_slug)
      const { count } = await supabase.from('purchase_history').select('*', { count: 'exact', head: true }).eq('user_id', user.id)
      setPurchaseCount(count || 0)
    } catch (e) { console.log('Product detail load error:', e) }
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.hero}>
          {PRODUCT_IMAGES[slug]
            ...
            : <View style={s.heroIconWrap}><Leaf size={56} color={Colors.alabaster} strokeWidth={1.5} /></View>}
        ...
        <View style={s.section}>
          ...
          <View style={s.infoCard}><Text style={s.infoText}>{productConfig.how}</Text></View>
        </View>
        ...
        <View style={s.section}>
          ...
          <View style={s.infoCard}><Text style={s.infoText}>{productConfig.when}</Text></View>
        </View>
        ...
        <View style={s.section}>
          ...
          <View style={s.infoCard}><Text style={s.infoText}>{productConfig.storage}</Text></View>
        </View>
        ...
        {productConfig.combo && (
          <View style={s.section}>
            ...
            <View style={[s.infoCard, { borderLeftColor: Colors.aloe }]}><Text style={s.infoText}>{productConfig.combo}</Text></View>
          </View>
        )}

        <View style={s.reminderCard}>
          <View style={s.reminderTitleRow}><Bell size={13} color={Colors.pine} strokeWidth={1.75} /><Text style={s.reminderTitle}>KUJTUES DITOR</Text></View>
...
const s = StyleSheet.create({
  ...
  hero: { backgroundColor: Colors.pine, paddingTop: 24, paddingBottom: 32, paddingHorizontal: 24, alignItems: 'center' },
  heroIconWrap: { width: 120, height: 120, borderRadius: 60, backgroundColor: 'rgba(255,255,255,0.08)', alignItems: 'center', justifyContent: 'center', marginBottom: 12 },
  ...
  loyaltyBadge: { marginTop: 14, flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: Colors.aloe + '30', borderRadius: 20, paddingHorizontal: 16, paddingVertical: 6 },
  ...
  infoCard: { backgroundColor: '#fff', borderRadius: 12, padding: 16, borderLeftWidth: 3, borderLeftColor: Colors.pine, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 4, elevation: 1 },
  infoText: { fontSize: 14, color: '#444', lineHeight: 22 },
  reminderCard: { marginHorizontal: 16, marginTop: 16, backgroundColor: Colors.pine + '10', borderRadius: 14, padding: 16, alignItems: 'center', borderWidth: 1, borderColor: Colors.pine + '20' },
  ...
  codeCard: { marginHorizontal: 16, marginTop: 16, backgroundColor: '#fff', borderRadius: 12, padding: 16, alignItems: 'center' },
  ...
  changeBtn: { marginHorizontal: 16, marginTop: 16, borderRadius: 12, borderWidth: 1.5, borderColor: Colors.pine + '40', padding: 14, alignItems: 'center' },
