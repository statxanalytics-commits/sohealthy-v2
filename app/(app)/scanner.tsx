...
function ratePlate(t: ScanResult['total']): PlateRating {
  ...
  if (t.sugar > 25) { score -= 25; issues.push(`sheqer i lartë (${t.sugar}g)`) } else if (t.sugar > 15) { score -= 12 }
  if (t.fat > 35) { score -= 20; issues.push(`yndyrna të larta (${t.fat}g)`) } else if (t.fat > 25) { score -= 10 }
  if (t.fiber < 2) { score -= 20; issues.push(`fibra shumë e ulët (${t.fiber}g)`) } else if (t.fiber < 4) { score -= 10 }
  if (t.protein < 10) { score -= 20; issues.push(`proteina shumë të ulëta (${t.protein}g)`) } else if (t.protein < 18) { score -= 10 }
...
async function saveScan(result: ScanResult, rating: PlateRating) {
  try {
    ...
    await supabase.from('scan_history').insert({
      user_id: user.id,
      food_name: result.items.map(i => i.name).join(', '),
...
export default function ScannerScreen() {
  ...
  async function pickImage(useCamera: boolean) {
    ...
    } catch (e) { setErrMsg('Gabim gjatë zgjedhjes së fotografisë.'); setState('error') }
  }
  ...
  async function analyzeImage(base64: string, mimeType: string) {
    ...
    setErrMsg(null)
    try {
      const response = await fetch(API.scanner, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ image: base64, mimeType }) })
      ...
      setResult(parsed); setPlate(rating); setState('result')
    } catch (e: any) { setErrMsg('Gabim gjatë analizës. Provo përsëri.'); setState('error') }
  }

  function goHome() { setState('home'); setImageUri(null); setResult(null); setPlate(null); setErrMsg(null) }
    ...
    <SafeAreaView style={s.safe} edges={['top']}>
      <View style={s.header}>
        ...
        <View style={s.titleRow}>
          <Camera size={18} color={Colors.alabaster} strokeWidth={1.75} />
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        {state === 'home' && (
          <View style={s.card}>
            <View style={s.plateIconWrap}>
              <Utensils size={44} color={Colors.pine} strokeWidth={1.5} />
            </View>
            ...
            <TouchableOpacity style={s.primaryBtn} onPress={() => pickImage(true)}>
              <Camera size={18} color={Colors.alabaster} strokeWidth={2} />
        ...
        {state === 'error' && (
          <View style={[s.card, s.centerCard]}>
            <View style={s.plateIconWrap}>
              <AlertTriangle size={44} color={Colors.goji} strokeWidth={1.5} />
            </View>
          ...
          <>
            ...
            <View style={s.calorieCard}>
              ...
              <View style={s.macroGrid}>
                {([['Proteina', result.total.protein], ['Karbohidrate', result.total.carbs], ['Yndyrna', result.total.fat], ['Fibra', result.total.fiber], ['Sheqeri', result.total.sugar]] as [string, number][]).map(([l, v]) => (
                  <View key={l} style={s.macroBadge}>
            ...
            {result.items.map((item, i) => (
              <View key={i} style={s.foodCard}>
                <View style={s.foodHeader}>
                  ...
                  <View style={s.kcalBadge}><Text style={s.kcalText}>{item.calories} kcal</Text></View>
                </View>
                <View style={s.macroRow}>
                  {([['P', item.protein], ['K', item.carbs], ['Y', item.fat], ['F', item.fiber]] as [string, number][]).map(([l, v]) => (
                    <View key={l} style={s.microBadge}><Text style={s.microText}>{l}: {v}g</Text></View>
                  ))}
                </View>
              </View>
            ))}
...
const s = StyleSheet.create({
  ...
  plateIconWrap: { width: 88, height: 88, borderRadius: 44, backgroundColor: Colors.pine + '10', alignItems: 'center', justifyContent: 'center', marginBottom: 16 },
  ...
  primaryBtn: { backgroundColor: Colors.pine, borderRadius: 12, padding: 15, alignItems: 'center', width: '100%', marginBottom: 10, flexDirection: 'row', justifyContent: 'center', gap: 8 },
  ...
  ratingScore: { fontSize: 18, fontWeight: '700' },
  ratingInfo: { flex: 1 },
