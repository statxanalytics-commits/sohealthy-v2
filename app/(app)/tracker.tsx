...
export default function TrackerScreen() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const { data: scanData } = await supabase
        .from('scan_history')
        .select('calories, protein_g, carbs_g, fat_g, scanned_at')
        .eq('user_id', user.id)
      ...
      scanData?.forEach(row => {
        ...
        byDate[date].carbs_g += row.carbs_g || 0
        byDate[date].fat_g += row.fat_g || 0
        byDate[date].count += 1
      })
      setScans(Object.values(byDate).slice(0, 14))

      // Load weight entries (stored as tracker_entries with product_slug = "weight:XX.X")
      const { data: weightData } = await supabase
        .from('tracker_entries')
        .select('id, date, product_slug')
        .eq('user_id', user.id)
  ...
  async function saveWeight() {
    ...
    try {
      ...
      // Upsert: if today already exists, update it
      ...
      Alert.alert('Gabim', 'Nuk u ruajt pesha. Provo përsëri.')
    } finally {
              ...
              <View style={s.macroRow}>
                {[['P', todayScan.protein_g], ['K', todayScan.carbs_g], ['Y', todayScan.fat_g]].map(([l, v]) => (
                  <View key={l as string} style={s.macroBadge}>
                    <Text style={s.macroL}>{l}</Text>
                    <Text style={s.macroV}>{Math.round(v as number)}g</Text>
                  </View>
                ))}
              </View>
            )}
          ...
          <View style={s.section}>
            ...
            {latestWeight ? (
              ...
              <View style={s.emptyCard}>
                <Text style={s.emptyText}>Ende nuk keni regjistruar peshën</Text>
              </View>
            )}
            {weights.length > 1 && (
                  ...
                  return (
                    <TouchableOpacity key={w.id} style={s.weightRow} onLongPress={() => deleteWeight(w.id)}>
                      ...
                      <View style={s.weightRowRight}>
                        {diff !== null && (
                          <Text style={[s.weightDiff, { color: diff < 0 ? Colors.aloe : Colors.goji }]}>
                ...
                return (
                  <View key={day.date} style={[s.dayRow, isToday && s.dayRowToday]}>
                    <View style={s.dayLeft}>
                      <Text style={s.dayDate}>{isToday ? 'Sot' : formatDate(day.date)}</Text>
                      <Text style={s.daySubs}>{day.count} skane</Text>
                    </View>
...
const s = StyleSheet.create({
  ...
  macroL: { fontSize: 10, color: Colors.aloe, marginBottom: 2 },
  macroV: { fontSize: 16, fontWeight: '700', color: '#fff' },
  ...
  weightUnit: { fontSize: 20, fontWeight: '400' },
  ...
  weightDiff: { fontSize: 12, fontWeight: '600' },
  weightRowVal: { fontSize: 14, fontWeight: '700', color: Colors.pine },
  ...
  dayLeft: { width: 90 },
  dayDate: { fontSize: 13, fontWeight: '600', color: Colors.pine },
