...
export default function SelectProductScreen() {
  ...
  useFocusEffect(useCallback(() => { checkExistingSelection() }, []))
  ...
  async function checkExistingSelection() {
    try {
      ...
      const { data: sel } = await supabase.from('product_selections').select('product_slug').eq('user_id', user.id).eq('order_code', profile.order_code).eq('is_active', true).single()
      if (sel?.product_slug) { setCurrentSelection(sel.product_slug); setSelected(sel.product_slug) }
    } catch (e) {}
  }
  ...
  async function confirmSelection() {
    ...
    try {
      ...
      const { data: profile } = await supabase.from('profiles').select('order_code').eq('id', user.id).single()
      if (!profile?.order_code) return
      await supabase.from('product_selections').update({ is_active: false }).eq('user_id', user.id).eq('order_code', profile.order_code)
      ...
      await supabase.from('purchase_history').update({ product_slug: selected }).eq('user_id', user.id).eq('order_code', profile.order_code)
            ...
            return (
              ...
              >
                ...
                {PRODUCT_IMAGES[p.slug]
                  ...
                  : <View style={s.productIconWrap}><Leaf size={32} color={Colors.aloe} strokeWidth={1.5} /></View>}
        ...
        <View style={s.footer}>
          ...
          >
            {saving
              ...
              : <Text style={s.confirmText}>Konfirmo — {PRODUCT_LIST.find(p => p.slug === selected)?.name} →</Text>
            }
          </TouchableOpacity>
        </View>
      )}
...
const s = StyleSheet.create({
  ...
  header: { backgroundColor: Colors.pine, paddingHorizontal: 20, paddingTop: 8, paddingBottom: 20, alignItems: 'center' },
  ...
  scroll: { padding: 16 },
  intro: { fontSize: 14, color: '#555', lineHeight: 22, textAlign: 'center', marginBottom: 20, backgroundColor: '#fff', borderRadius: 12, padding: 16, borderLeftWidth: 3, borderLeftColor: Colors.aloe },
  grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  productCard: { width: '47%', backgroundColor: '#fff', borderRadius: 14, padding: 16, alignItems: 'center', borderWidth: 2, borderColor: 'transparent', shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 6, elevation: 2, position: 'relative' },
  productCardSelected: { borderColor: Colors.pine, backgroundColor: Colors.pine + '08' },
  checkBadge: { position: 'absolute', top: 8, right: 8, width: 22, height: 22, borderRadius: 11, backgroundColor: Colors.pine, alignItems: 'center', justifyContent: 'center' },
  productIconWrap: { width: 80, height: 80, borderRadius: 40, backgroundColor: Colors.aloe + '15', alignItems: 'center', justifyContent: 'center', marginBottom: 8 },
  ...
  footer: { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: Colors.alabaster, padding: 16, paddingBottom: 32, borderTopWidth: 1, borderTopColor: '#e0e0e0' },
  confirmBtn: { backgroundColor: Colors.pine, borderRadius: 14, padding: 16, alignItems: 'center' },
